...
public class ABanListener implements Listener {

    private final KushStaffUtils instance;

    public ABanListener(KushStaffUtils instance) {
    ...
    private void sendInfoToDiscord(Punishment punishment) {
        ...
        String webhookUrl = StringUtils.format(config.getString("advancedbans.ban.webhookUrl"));
        String embedTitle = StringUtils.format(config.getString("advancedbans.ban.embedTitle"));
        String embedDescription = StringUtils.format(config.getString("advancedbans.ban.embedDescription").replace("%time%", "<t:" + time + ":R>"));
        String embedFooter = StringUtils.format(config.getString("advancedbans.ban.embedFooter").replace("%time%", (new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date())));
        String thumbnailUrl = StringUtils.format(config.getString("advancedbans.ban.embedThumbnail"));
        String avatarUrl = StringUtils.format(config.getString("advancedbans.ban.embedAvatar"));
