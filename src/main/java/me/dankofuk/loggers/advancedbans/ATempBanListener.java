...
public class ATempBanListener implements Listener {

    private final KushStaffUtils instance;

    public ATempBanListener(KushStaffUtils instance) {
    ...
    public void onPunishment(PunishmentEvent event) {
        ...
        if (punishment.getType().equals(PunishmentType.TEMP_BAN)) {
    ...
    private void sendInfoToDiscord(Punishment punishment) {
        ...
        String webhookUrl = StringUtils.format(config.getString("advancedbans.tempban.webhookUrl"));
        String embedTitle = StringUtils.format(config.getString("advancedbans.tempban.embedTitle"));
        String embedDescription = StringUtils.format(config.getString("advancedbans.tempban.embedDescription").replace("%time%", "<t:" + time + ":R>"));
        String embedFooter = StringUtils.format(config.getString("advancedbans.tempban.embedFooter").replace("%time%", (new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date())));
        String thumbnailUrl = StringUtils.format(config.getString("advancedbans.tempban.embedThumbnail"));
        String avatarUrl = StringUtils.format(config.getString("advancedbans.tempban.embedAvatar"));
