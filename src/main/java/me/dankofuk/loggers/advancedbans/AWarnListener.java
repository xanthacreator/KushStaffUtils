...
public class AWarnListener implements Listener {

    private final KushStaffUtils instance;

    public AWarnListener(KushStaffUtils instance) {
    ...
    public void onPunishment(PunishmentEvent event) {
        ...
        if (punishment.getType().equals(PunishmentType.WARNING) || punishment.getType().equals(PunishmentType.TEMP_WARNING)) {
    ...
    private void sendInfoToDiscord(Punishment punishment) {
        ...
        String webhookUrl = StringUtils.format(config.getString("advancedbans.warn.webhookUrl"));
        String embedTitle = StringUtils.format(config.getString("advancedbans.warn.embedTitle"));
        String embedDescription = StringUtils.format(config.getString("advancedbans.warn.embedDescription").replace("%time%", "<t:" + time + ":R>"));
        String embedFooter = StringUtils.format(config.getString("advancedbans.warn.embedFooter").replace("%time%", (new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date())));
        String thumbnailUrl = StringUtils.format(config.getString("advancedbans.warn.embedThumbnail"));
        String avatarUrl = StringUtils.format(config.getString("advancedbans.warn.embedAvatar"));
